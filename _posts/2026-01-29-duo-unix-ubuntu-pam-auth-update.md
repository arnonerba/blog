...
---
Enabling [Duo Unix](https://duo.com/docs/duounix) on Ubuntu involves integrating the `pam_duo` module into your system's [Pluggable Authentication Modules (PAM)](https://en.wikipedia.org/wiki/Pluggable_Authentication_Module) stack[^1]. This can be done by manually editing the files in `/etc/pam.d/`, but Ubuntu provides a safer way to do this in the form of `pam-auth-update` and its associated PAM config framework.

## Ubuntu's PAM Config Framework

The [pam-auth-update](https://manpages.ubuntu.com/manpages/noble/en/man8/pam-auth-update.8.html) script and its associated [PAMConfigFrameworkSpec](https://wiki.ubuntu.com/PAMConfigFrameworkSpec) were [originally written by Steve Langasek](https://raphaelhertzog.com/2011/05/06/people-behind-debian-steve-langasek-release-wizard/)[^2] and were added to Debian and its derivatives in 2008[^3].
...
The PAM module for Duo Unix falls into the `auth` management group, so, for the purposes of this integration, that's the only applicable module type.
...
These are the files that `pam-auth-update` manages. For context, our PAM profile for Duo Unix will only affect `/etc/pam.d/common-auth`. With all this in mind, we can move on to writing the profile.
...
**Note:** The third option will overwrite your current system-wide PAM configuration without prompting, which should be safe unless you have manually edited the aforementioned system-wide PAM configuration files in `/etc/pam.d`.
...
[^1]: The legacy `login_duo` approach for enabling Duo Unix is outside the scope of this post.
[^2]: Tragically, [Steve passed away early last year](https://discourse.ubuntu.com/t/remembering-and-thanking-steve-langasek/52665).
[^3]: Those of you who are familiar with other distros may have noted that Fedora and its derivatives use [authselect](https://github.com/authselect/authselect) (which replaces the older `authconfig` utility) instead.
[^4]: For example, this might be done to allow both local and domain accounts to log into a system.
[^5]: Hint: This can be accomplished by editing the package-provided `sshd` PAM stack at `/etc/pam.d/sshd`, but this carries its own risks. Caveat emptor.
